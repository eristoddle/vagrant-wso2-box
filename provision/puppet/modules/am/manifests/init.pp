class am{
  ...
  file { '/etc/init.d/wso2am':
    ...
    group  => root,
    mode   => 755,
    source => '/vagrant/provision/puppet/modules/am/files/wso2am',
  }->
  service { 'wso2am':
    ensure => true,
    enable => true,
  ...
  file { '/opt/wso2am-1.7.0/repository/conf/api-manager.xml':
    source  => '/vagrant/provision/puppet/modules/am/files/api-manager.xml',
  ...
  file { '/opt/wso2am-1.7.0/repository/conf/datasources/master-datasources.xml':
    source  => '/vagrant/provision/puppet/modules/am/files/master-datasources.xml',
    require => Exec['Extract WSO2 API Manager'],
    notify  => Service['wso2am'],
  }
}